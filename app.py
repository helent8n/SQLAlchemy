...
def welcome():
   ...
   <a href="/api/v1.0/stations">/api/v1.0/stations</a>
    ...
    <br>
    <a href="/api/v1.0/2017-01-01/2017-01-14">/api/v1.0/2017-01-01/2017-01-14</a>
    </li>
    <br>
    </ul>
    </html>
...
def precipitation():
    ...
    prcp_12 = session.query(Measurement.date, Measurement.prcp).\
              filter(Measurement.date >= year_ago_date).filter(Measurement.date <= recent_date).\
                     order_by(Measurement.date).all()


    # Convert list of tuples into normal list
    prcp_dict = dict(prcp_12)
...
def tobs(): 
    ...
    year_ago_date = dt.datetime.strptime(recent_date, "%Y-%m-%d") - dt.timedelta(days=365)
    # Query tobs
    results_tobs = session.query(Measurement.date, Measurement.tobs).filter(Measurement.date >= year_ago_date).all()
    # Convert list of tuples into normal list
